...
CLASSES = [
    ...
    # 必填缺失必须排在 C 前面：它的原文同样含「参数校验失败」，但那是模型漏传参数，
    # 改 schema 类型声明治不了。混进 C 会让 C 的 count 永远归不了零（4/9 是不可修的），
    # 而重放只覆盖得了可修的那 5 次 → total<count 永远成立 → feasible 钉在 1.0。
    ("F", "必填参数缺失", ("缺少必填参数",),
     "模型漏传必填参数：给该参数默认值，或把必填项写进工具描述/调用示例"),
    ("C", "参数类型错", ("参数校验失败",),
