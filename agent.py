...
# ---- 「说重点」节奏面埋点：工具轮里有没有说话 ----
# 这条规则此前只测套话率（说的是什么），测不到节奏（该说的时候说没说）：全程沉默
# 在数据里完全隐形，用户那边看到的就是黑箱。「说了话」的判据可数：该轮正文剥掉
# think 标记与工具协议块后 >= 12 字符（「我来看看」4 字、空文本都不算）。
MID_SPEECH_MIN_CHARS = 12
    ...
    async def _chat_stream_normal(self, message: str, history: list = None,
        ...
        if tools:
            agent_rules = (
                ...
                # 实测 377 轮里 tool_rounds 中位数 4、最长 115 轮，而「阶段」由模型自己划——
                # 不可判定的要求输给可判定的禁令，中间话就一律省了。触发条件改成可数事件
                # （连续 3 轮工具调用没说话），并由 mid_speech 埋点给出可校准的读数。
                #
                # 2026-09-25 二次修正：可数事件只是下限（不憋死），管不了「有发现自然就说」。
                # 原句的事件判据「有启发性发现」要我在结果回来的瞬间先做一次元判断（这算不算
                # 发现），而同句禁令有现成反例词——元判断一犹豫就默认沉默。判据因此换成结果
                # 自带的硬特征（报错/预期不符/上一步判断被推翻），并写明默认倾向：拿不准时
                # 倾向说一句短的事实，而不是咽回去。
                "⚠ 说重点（硬规则）：用户看不到你的工具调用和思考，只看得到文字——开工先说一句要做什么；"
                "过程中每条 1~2 句，说「刚拿到什么、意味着什么、下一步」；"
                "不播报无信息量进展（「我来看看」），但报错、结果不符预期、上一步判断被推翻、方向要变，当场说；"
                "拿不准就倾向说一句短事实，别咽回去；连续 3 轮没说话必须先补一句；"
                "轮末 1~2 句先结论后细节；卡住或需要你决策立刻开口；\n"
                "工具详细用法与更多能力见下方技能说明。\n\n"
            )
        ...
        eff_seen: set = set()
        # 「说重点」节奏面：工具轮里说过话的轮数 + 最长连续静默工具轮。
        # 断点续跑只恢复 full_text，这两个计数器从 0 重数，所以审计的分母取
        # 「带埋点的轮」而不是全量轮——不拿半个现场当完整现场。
        mid_speech_rounds = 0
        silent_streak = 0
        silent_max = 0
        full_text = ""
        ...
        def _record_turn_metrics():
            ...
            try:
                ...
                _rec(message, {
                    ...
                    # 「说重点」节奏面：工具轮里说过话的轮数 / 最长连续静默工具轮。
                    # 分母是同一条记录里的 tool_rounds——两者同源，比例才有意义。
                    "mid_speech_rounds": mid_speech_rounds,
        ...
        while max_tool_rounds <= 0 or tool_round < max_tool_rounds:
            ...
            # 过程话埋点：用户看不到工具调用，只能看到正文——「这一轮说没说话」是
            # 「说重点」唯一可数的节奏观测。剥掉 think 标记与文本协议块再量长度，
            # 否则 <tool_call> 里的 JSON 会被算成「说了话」。
            _spoken = _strip_think_markers(assistant_content or "").strip()
            if text_tool_call:
                _spoken = TEXT_TOOL_CALL_STRIP_RE.sub("", _spoken).strip()
            if len(_spoken) >= MID_SPEECH_MIN_CHARS:
                mid_speech_rounds += 1
                silent_streak = 0
            else:
                silent_streak += 1
                silent_max = max(silent_max, silent_streak)
