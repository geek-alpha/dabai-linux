...
FAVS_FILE = BASE_DIR / 'video_favorites.json'
...
def _save(data: dict) -> None:
    tmp = str(FAVS_FILE) + '.tmp'
    with open(tmp, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    try:
        tmp_path = Path(tmp)
        tmp_path.replace(FAVS_FILE)  # 原子替换，防写一半损坏
    except Exception:
...
def get_favorite(fid: str) -> dict | None:
    with _save_lock:
        for f in _load()['favorites']:
            if f['id'] == fid:
                return f
    return None
...
def add_favorite(video: dict, category_id: str | None = None) -> dict:
...
def remove_favorite(fid: str) -> bool:
    with _save_lock:
        data = _load()
...
def move_favorite(fid: str, category_id: str | None) -> bool:
