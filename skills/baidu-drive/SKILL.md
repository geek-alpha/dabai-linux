...
本技能装在大白（dabai）里，上面这份 SKILL.md 是上游原文（Claude Code 风格：Agent 自己拼 shell）。
大白这边的入口是 `skill.py` 里的 3 个工具，**优先用工具，别直接拼 shell**：

| 工具 | 用途 | 对应上游 |
|---|---|---|
| `bdpan_status()` | 查安装/版本/登录态 | 前置检查 1、2 |
| `bdpan_login(code?)` | 不带 code → 返回授权链接；带 code → 完成登录 | `bash scripts/login.sh` |
| `bdpan(args=[...], confirm?, timeout?, background?)` | 全部文件操作 | 各 `bdpan <cmd>` 调用 |

- 公共参数 `--agentname/--session-id/--session-input` 由工具自动注入，不要手写；
- `bdpan login*`、`logout`、`update`、`install` 在通用通道被拒（登录只走 `bdpan_login`）；
- `rm` 工具层强制要求 `confirm=true`（先列清单等用户确认）；
- 大文件传输传 `background=true`，返回 PID + 日志路径，再用 `tail` 查进度；
- 二进制装在 `~/.local/bin/bdpan`（官方安装器），工具内部已把它补进 PATH。
- 非交互环境没有 TTY：`login.sh` 的安全确认由工具喂 `y`，`rm` 自动补 `--force`。
